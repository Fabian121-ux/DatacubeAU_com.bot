...
def test_parse_exact_owner_style_instruction_with_named_date_and_time():
    ...
    plan = NaturalActionPlannerService.parse(
        "@Zina message Amanda Christabel at 9:00am on Aug 25 and tell her the document is ready",
...
def test_parse_tomorrow_instruction_with_colon_message_body_does_not_split_time_colon():
    ...
    plan = NaturalActionPlannerService.parse(
        "message Amanda at 9:00am tomorrow: the document is ready",
        timezone="Africa/Lagos",
        now=now,
    )
...
def test_parse_rejects_past_explicit_time():
    ...
    with pytest.raises(ValueError, match="future"):
        NaturalActionPlannerService.parse(
            "message Amanda at 9am on Aug 24 2026 and tell her hello",
            timezone="Africa/Lagos",
            now=now,
        )
...
async def test_create_from_instruction_reuses_contact_intelligence_and_scheduler(db_session):
    ...
    row = (
        await db_session.execute(
            select(ScheduledAction).where(ScheduledAction.idempotency_key == "natural-amanda-test")
        )
    ).scalar_one()
