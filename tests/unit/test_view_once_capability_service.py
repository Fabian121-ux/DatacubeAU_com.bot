...
def test_noweb_key_is_view_once_is_explicit_marker_without_becoming_source_identity():
    capability = ViewOnceCapabilityService.inspect_message_payload(
        {
            "id": "false_222@c.us_OUTER",
            "key": {"id": "INNER", "isViewOnce": True},
        }
    )

    assert capability.source_message_id == "false_222@c.us_OUTER"
    assert capability.is_view_once is True
    assert capability.retrievable_now is False


def test_audio_mime_is_classified_only_when_explicit_view_once_evidence_exists():
    capability = ViewOnceCapabilityService.inspect_reply_snapshot(
        {
            "replyTo": {
                "id": "MSG-AUDIO",
    ...
    assert capability.source_message_id == "MSG-AUDIO"
    assert capability.is_view_once is True
    assert capability.media_mime == "audio/ogg"
    assert capability.media_type == "audio"
    assert capability.retrievable_now is True


def test_plain_audio_media_is_not_promoted_to_view_once():
    capability = ViewOnceCapabilityService.inspect_reply_snapshot(
        {
            "replyTo": {
                "id": "MSG-AUDIO-PLAIN",
